...
from typing import Any, Callable, NamedTuple, Optional, List, Dict
    ...
    def __init__(
        ...
        if ms_layers:
            ...
            for layer_name in ms_layers:
                assert layer_name in [f"layer{i+1}" for i in range(num_layers)]
            print(
                f"Insert {self.mixstyle.__class__.__name__} after {ms_layers}"
            )
        self.ms_layers = ms_layers

        self.perturbation = perturbation(p=uncertainty) if dsu_layers else None
        ...
        for i in range(num_layers):
            layers[f"encoder_layer_{i}"] = EncoderBlock(
                ...
                attention_dropout,
                norm_layer,
                self.mixstyle if f"layer{i+1}" in self.ms_layers else None,
                self.perturbation if f"layer{i+1}" in self.dsu_layers else None
            )
...
@BACKBONE_REGISTRY.register()
def vit_b_16_ms(*, weights: Optional[ViT_B_16_Weights] = None, progress: bool = True, **kwargs: Any) -> VisionTransformer:
    from dassl.modeling.ops import MixStyle_ViT
    ...
    return _vision_transformer(
        ...
        weights=weights,
        progress=progress,
        ms_class=MixStyle_ViT,
        ms_layers=[f"layer{i+1}" for i in range(11)],
        **kwargs,
    )


@BACKBONE_REGISTRY.register()
def uvit_b_16(*, weights: Optional[ViT_B_16_Weights] = None, progress: bool = True, **kwargs: Any) -> VisionTransformer:
    from dassl.modeling.ops import DistributionUncertainty_ViT
    ...
    return _vision_transformer(
        ...
        weights=weights,
        progress=progress,
        perturbation=DistributionUncertainty_ViT,
        dsu_layers=[f"layer{i+1}" for i in range(12)],
        **kwargs,
    )
